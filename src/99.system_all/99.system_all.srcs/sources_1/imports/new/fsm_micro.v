...
module fsm_micro(
    ...
    always @(posedge clk or posedge reset) begin
        if (reset || !enable) begin // Add enable to reset condition
            current_state <= IDLE;
    ...
    always @(*) begin
        ...
        if (!enable) begin // If not enabled, all outputs are off/inactive
            // Outputs are already set to default inactive values above
        end else begin
            case (current_state)
                IDLE: begin
